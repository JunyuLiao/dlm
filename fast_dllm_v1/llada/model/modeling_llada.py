...
try:
    from dllm.attention.blasst import dispatch_scaled_dot_product_attention
except ImportError:
...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        """
        runtime = getattr(self, "_dllm_attention_runtime", None)
        if runtime is not None and runtime.config.enable_blasst_2d:
            if k.size(1) != q.size(1):
                if q.size(1) % k.size(1):
                    raise ValueError("query heads must be divisible by KV heads")
                repeats = q.size(1) // k.size(1)
                k = k.repeat_interleave(repeats, dim=1, output_size=q.size(1))
                v = v.repeat_interleave(repeats, dim=1, output_size=q.size(1))
