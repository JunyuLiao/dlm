...
如果你遇到 `LLaDAModelLM object has no attribute all_tied_weights_keys`，不要用 `device_map=auto`；当前脚本默认 `DEVICE_MAP=none`，会先加载模型再 `model.to(cuda)`，单张 H100 可以放下 LLaDA-8B。
