...
ROOT=Path('results/diffusion_gemma_jl_lowrank_aime30_longbench100_v11')
MODULE='experiments.diffusion_gemma_jl_lowrank_multibench_v2'
AIME=tiny.ROOT
...
def prepare(root=ROOT):
    ...
    setup=dict(schema='jl_lowrank_multibench_v11',**{k:deepcopy(old[k]) for k in
...
def execution(root=ROOT):
    ...
    files=(Path(__file__),Path(reporting.__file__),Path('tests/test_jl_lowrank_multibench_v2.py'),tests,root/'setup.json')
    sources=backend.evidence.merge_sources(sources,{str(p):sha(p.read_bytes()) for p in files});fb.check_sources(sources)
    data=dict(schema='jl_lowrank_multibench_execution_v11',sources=sources,runtime=runtime,
...
def validate_cached_condition(config,name,benchmark,threshold):
    expected={} if name=='dense' else CONFIGS[name]
    # Legacy dense metadata has thresholds={}; no routing threshold is applied.
    # A missing sparse threshold cannot pass because its expected policy is not None.
    if config['config']!=expected or config['thresholds'].get(benchmark)!=threshold:
        raise ValueError('Cached output operator/threshold mismatch')
    return expected
    ...
    for b,labels in REUSED_BY_BENCHMARK.items():
        for label in labels:
            ...
            for row in (r for r in setup['final'] if r['benchmark']==b):
                ...
                expected=validate_cached_condition(cfg,name,b,threshold)
...
def regenerate(root=ROOT):
    from experiments.diffusion_gemma_jl_lowrank_multibench_v2_report import regenerate as report
    return report(root)
