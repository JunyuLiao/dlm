...
> **Repository layout:** this historical implementation now lives under
> `fast_dllm_v2/`. New model-independent inference, BLASST integration, and
> exact-count RULER evaluation live in the root `dllm` package and
> `scripts/ruler/`. The long experiment programs below are retained for paper
> reproduction; `scripts/eval_blasst_ruler.py` forwards to the shared CLI.
